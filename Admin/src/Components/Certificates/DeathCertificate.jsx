...
export default function DeathCertificate() {
  ...
  if (loading) {
    return <p className="text-center mt-10">Loading death certificates...</p>;
  }
        ...
        <table className="min-w-full bg-white shadow-md rounded-lg">
          ...
          <thead className="bg-gray-200 text-gray-700 text-xs lg:text-sm">
            <tr>
              ...
              <th className="px-3 py-2">Marathi Name</th>
          ...
          <tbody>
            {data.map((item) => (
              <tr key={item._id} className="border-t text-xs lg:text-sm hover:bg-gray-50">
                ...
                <td className="px-3 py-2">{item.deceasedName}</td>

                <td className="px-3 py-2">
                  {new Date(item.deathDate).toLocaleDateString()}
                </td>

                <td className="px-3 py-2">{item.deathTime || "-"}</td>
                <td className="px-3 py-2">{item.deathReason}</td>
