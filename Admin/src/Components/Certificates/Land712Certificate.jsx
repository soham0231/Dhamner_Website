...
export default function Land712Certificate() {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);

  // ✅ YOUR BACKEND ROUTE
  const API = "http://localhost:5000/api/land712";
  ...
  if (loading) {
    return <p className="text-center mt-10">Loading Land 7/12 records...</p>;
  }

  return (
    <div className="p-4 lg:p-8">
      <h2 className="text-2xl font-bold mb-6 text-center">
        Land 7/12 Applications
      </h2>
          ...
          <tbody>
            {data.map((item) => (
              <tr key={item._id} className="border-t text-xs lg:text-sm hover:bg-gray-50">
                ...
                <td className="px-3 py-2">{item.surveyNo}</td>
