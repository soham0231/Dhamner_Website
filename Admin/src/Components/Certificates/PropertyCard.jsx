...
export default function PropertyCard() {
  ...
  if (loading) {
    return <p className="text-center mt-10">Loading property cards...</p>;
  }

  return (
    <div className="p-4 lg:p-8">
      <h2 className="text-2xl font-bold mb-6 text-center">
        Property Card Applications
      </h2>
        ...
        <table className="min-w-full bg-white shadow-md rounded-lg">
          ...
          <thead className="bg-gray-200 text-xs lg:text-sm">
            <tr>
              ...
              <th className="px-3 py-2">CTS No</th>
              <th className="px-3 py-2">ULPIN</th>
          ...
          <tbody>
            {data.map((item) => (
              <tr key={item._id} className="border-t text-xs lg:text-sm hover:bg-gray-50">
                ...
                <td className="px-3 py-2">{item.ctsNo}</td>
                <td className="px-3 py-2">{item.ulpin}</td>
